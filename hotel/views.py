...
from .models import Comment, Order, Food, Data, Cart, OrderContent, Staff, DeliveryBoy
...
def dashboard_admin(request):
    ...
    customers = UserProfile.objects.count()
    completed_orders = Order.objects.filter(payment_status="Completed")
    top_customers = UserProfile.objects.filter().order_by('-total_sale')
...
def add_user(request):
    if request.method == "POST":
        ...
        if password == confirm_pass:
            ...
            user.save()
            cust = UserProfile.objects.create(customer=user, address=address, contact=contact)
            cust.save()
...
def placeOrder(request):
    to_email = []
    customer = UserProfile.objects.get(customer=request.user)
...
def my_orders(request):
    user = User.objects.get(id=request.user.id)
    customer = UserProfile.objects.get(customer=user)
